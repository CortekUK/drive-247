...
import { ContactForm } from "@nw/components/forms/contact-form";
import { DEFAULT_CONTACT_HERO, DEFAULT_SITE_SOCIAL } from "@nw/lib/cms/defaults";
import { loadSection } from "@nw/lib/cms/server";
import { socialLinks } from "@nw/lib/cms/social";
...
export async function ContactHeroSection() {
  ...
  return (
    <section {...cmsSection("contact.hero", "Hero")} className="relative isolate -mt-[88px] bg-brand-cream">
        ...
        <div className="flex flex-col gap-8 lg:h-full">
          ...
          <div className="flex max-w-[520px] flex-col gap-4">
            <h1 className="text-3xl font-semibold leading-tight tracking-tight text-brand-text sm:text-4xl lg:text-5xl lg:leading-[1.05]">
              <Editable path="contact.hero.title">{hero.title}</Editable>
            </h1>
            <p className="text-sm leading-relaxed text-brand-text-soft sm:text-base">
              <Editable path="contact.hero.subtitle">{hero.subtitle}</Editable>
            </p>
          </div>
