...
import { useCmsSection } from "@nw/hooks/use-cms";
import { usePromotions } from "@nw/hooks/use-promotions";
import {
  DEFAULT_PROMOTIONS_EMPTY_STATE,
} from "@nw/lib/cms/defaults";
import { isRemoteImage } from "@nw/lib/cms/format";
import type { PromotionsResult } from "@nw/lib/cms/queries";
import type {
  EmptyStateContent,
  PageSections,
  PromoAccent,
  PromoItem,
} from "@nw/lib/cms/types";
  ...
  return (
    <div className="mx-auto mt-10 flex max-w-xl flex-col items-center gap-4 rounded-[14px] border border-brand-border-soft bg-brand-cream px-6 py-12 text-center">
      <h3 className="text-xl font-semibold text-brand-text">
        <Editable path="promotions.empty_state.title_active">{content.title_active}</Editable>
      </h3>
      <p className="max-w-[420px] text-sm leading-relaxed text-brand-text-soft">
        <Editable path="promotions.empty_state.description">{content.description}</Editable>
      </p>
      <Link
        href="/fleet"
      ...
      >
        <Editable path="promotions.empty_state.button_text">{content.button_text}</Editable>
      </Link>
    </div>
  );
}
