...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('BOOKING ESIGN API (BoldSign)');
        console.log('='.repeat(50));
        ...
        // Fallback: Get tenant from vehicle
        ...
        if (tenantId) {
            const templateCategory = 'standard';
        ...
        if (!brandId && tenantId && tenant?.company_name) {
            try {
                ...
                // Fetch tenant logo or generate placeholder
        ...
        return NextResponse.json({ ok: true, envelopeId: documentId, agreementId, emailSent });
