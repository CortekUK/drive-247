    ...
    <div style={containerStyle} className={printable ? "" : "space-y-6"}>
        ...
        groups.map((g: StatementGroup) => {
          ...
          return (
            ...
            >
              {/* Group header */}
              <div
                style={{ padding: "10px 12px", background: "#f3f4f6", display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}
              >
      ...
      >
        <p style={printable ? { margin: 0 } : undefined}>
          This is a statement of account, not a tax invoice — see your individual rental invoices for itemised tax.
        </p>
...
export function AccountStatementDialog({ open, onOpenChange }: Props) {
  const { tenant } = useTenant();
  const { customerUser } = useCustomerAuthStore();
  const { data, isLoading, error } = useCustomerStatement(open);
  ...
  const statementNumber = useMemo(() => {
    const token = (customerUser?.customer_id ?? "").replace(/-/g, "").slice(0, 6).toUpperCase() || "ACCT";
    return `STMT-${format(generatedAt, "yyyyMMdd")}-${token}`;
  }, [customerUser?.customer_id, generatedAt]);
  ...
  return (
    <>
      {/* Hidden printable copy — inline styles only, so print fidelity isn't tied to Tailwind */}
      {hasData && (
        ...
        >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-16 text-center text-sm text-muted-foreground">
              ...
              Could not load your statement. Please try again.
            </div>
          ) : data ? (
            <StatementContent
              data={data}
