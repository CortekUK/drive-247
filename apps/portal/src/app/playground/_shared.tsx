...
/**
 * Shared vocabulary for the design playground.
 *
 * Everything in `app/playground/*` is fake — no Supabase, no tenant, no auth.
 *
 * The visual grammar here is NOT invented: it is northwind's, taken from
 ...
 *
 * Not a nav label — a record of a decision. Before you decide, it asks its
 * question ("Who is renting?") on a flat grey ground. After you decide, the
 * question is replaced by your answer and the block takes its share of the
 * accent. The grey is permanent, not a hover state: an empty stage is a real
 * state and it should be visible without touching anything.
 *
...
export function StageItem({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      style={filled ? { backgroundColor: `hsl(var(--primary) / ${tintFor(index)})` } : undefined}
      className={cn(
        "w-full cursor-pointer rounded-2xl px-3.5 py-2.5 text-left transition-all",
 ...
 *
 * The screen is a fixed frame — nothing scrolls the page itself. Each of the
 * three columns is its own viewport-height column, and only the CONTENT inside
 * one moves. So the stage's title, and anything a tab pins via `toolbar`, stay
 * put while the list under them runs; the operator never scrolls a title out of
 * view to reach a row, and never loses the search box they are typing into.
 */
...
/** Thin wrapper on ui-v2's Badge so the sandbox's four tones map to one place. */
...
/** Kept so the three screens share one action button call-site shape. */
export function ActionButton({
  ...
  return (
    <Button variant={variant === "primary" ? "default" : "outline"} onClick={onClick} disabled={disabled}>
      {children}
    </Button>
  );
}
