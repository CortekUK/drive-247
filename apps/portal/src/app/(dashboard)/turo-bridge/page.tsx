...
export default function TuroBridgePage() {
  ...
  // THE CANARY GATE, and it is checked before the operator's own switch.
  //
  // `tenants.turo_bridge_enabled` is already `true` for five tenants in
  // production — jangramrentals and nealcorentals among them, from the PoC.
  // Gating only on that column would have handed this screen to two live
  // operators the moment it deployed. See V2_AREAS.turo in lib/v2.ts.
  //
  // A context read, not a query: it cannot fail, costs nothing, and resolves
  // to false for an unknown tenant, so the refusal is the safe direction.
  ...
  // Order matters, and the two refusals say DIFFERENT things on purpose.
  // `TuroSyncOff` tells the reader a colleague switched this off and that an
  // admin can switch it back on in Settings. For a tenant who is not on the
  // canary that sentence is simply untrue — nobody switched anything off, and
...
/**
 * Shown to a tenant who is not on the canary for this area.
 *
 * Deliberately states nothing the reader can act on, because there is nothing:
 * no setting of theirs is involved and no admin of theirs can change it. It
 * does not apologise, does not describe the feature as broken, and does not
 * promise a date.
 */
function TuroSyncUnavailable() {
  return (
    <div className="northwind mx-auto w-full max-w-[1200px] space-y-6 px-1 pb-6 pt-6">
      ...
      <EmptyState
        ...
        body={
          <p>
            Turo Sync is being rolled out gradually and is not switched on for this account
            yet. Nothing is missing from your fleet or your bookings — this page simply has
            nothing to show you. Talk to us if you would like to be part of the rollout.
          </p>
        }
      />
    </div>
