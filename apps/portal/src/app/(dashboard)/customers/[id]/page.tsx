...
const CustomerDetail = () => {
  ...
  const { data: latestVerification } = useQuery({
    ...
    queryFn: async () => {
      ...
      //
      // No provider filter: this panel is the only place a customer's identity
      // history surfaces, so it must render whatever provider wrote the row.
      // Rows from the retired third-party licence-check provider used to be
      // excluded here because that provider had its own dedicated tab; with the
      // tab gone, they would otherwise be invisible.
      ...
      // Fallback to the latest record of any provider if no completed one exists.
