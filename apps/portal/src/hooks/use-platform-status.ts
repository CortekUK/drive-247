...
export function usePlatformStatus(): PlatformStatus {
  ...
  // --- INSHUR Period Z ---
  // Hidden from the lean canary and that tenant alone. This gate cannot be
  // data-driven: the `else` branch below is the `comingSoon` teaser, which
  ...
  // Lean gate: drop the INSHUR row entirely — neither the configured variant nor
  // the `comingSoon` teaser reaches a lean tenant. Done here rather than inside
  // the array literal above because a conditional spread there would strip the
  // literal's contextual type and widen every string in the WHOLE array.
  const visibleChecklist = inshurHidden
    ? checklist.filter((i) => i.id !== "inshur")
    : checklist;

  const activeItems = visibleChecklist.filter((i) => !i.comingSoon);
