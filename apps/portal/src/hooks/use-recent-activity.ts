...
export const useRecentActivity = (allowedTypes?: string[]) => {
  const { tenant } = useTenant();

  return useQuery({
    queryKey: ["recent-activity", tenant?.id, allowedTypes],
    queryFn: async () => {
      const activities: ActivityItem[] = [];

      // Fetch recent payments
      if (!allowedTypes || allowedTypes.includes('payment')) {
      ...
      if (!allowedTypes || allowedTypes.includes('rental')) {
      ...
      if (!allowedTypes || allowedTypes.includes('vehicle')) {
      ...
      if (!allowedTypes || allowedTypes.includes('system')) {
