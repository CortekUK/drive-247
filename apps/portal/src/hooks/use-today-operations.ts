 ...
 *
 * TENANT ISOLATION: RLS is OFF on `rentals` (V2_PLAN §5). Every one of the four
 * queries below carries `.eq('tenant_id', tenant.id)` explicitly, and the hook
 * is `enabled` only once a tenant is resolved. Do not remove either.
 */
 ...
 *
 * Verified against production: `rentals.status` is free text carrying exactly
 * four values — Closed, Active, Pending, Cancelled.
 */
 ...
 *
 * Checked against production before picking the number: of the live rentals
 * whose end_date has passed, most are more than a month old and many are past
 * 90 days. A car is not actually missing for three months — those are rentals
 * that were never closed out. Listing them would bury the two or three
 * genuinely late vehicles and teach the operator to ignore the panel.
 *
