...
export function useCmdVerification(customerId: string | undefined) {
  ...
  // Lean tenants are not offered CheckMyDriver. Gating the QUERY (not just the
  // markup) means the canary never issues the read at all, so `hasCmd` on the
  // customer page resolves false and the whole CMD block collapses to the
  ...
  return useQuery({
    ...
    enabled: !!customerId && !!tenant?.id && !cmdHidden,
    refetchInterval: (q) => {
...
export function useCmdResults(applicantVerificationId: string | null | undefined) {
  ...
  return useQuery({
    ...
    enabled: !!applicantVerificationId && !cmdHidden,
    staleTime: 0,
    refetchOnWindowFocus: false,
  });
}
