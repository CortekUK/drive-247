...
export function useBonzahPremium({
  ...
  const hasRequiredParams = Boolean(tripStartDate && tripEndDate && pickupState);
  ...
  ], [tenant?.id, tripStartDate, tripEndDate, pickupState, debouncedCoverage]);

  const query = useQuery<BonzahPremiumResult>({
    queryKey,
    queryFn: async (): Promise<BonzahPremiumResult> => {
      if (!hasCoverage) {
        return { total_premium: 0, breakdown: { cdw: 0, rcli: 0, sli: 0, pai: 0 } };
      }
