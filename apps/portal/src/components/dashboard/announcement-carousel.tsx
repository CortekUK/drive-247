      ...
      <DialogContent className="max-w-lg">
        ...
        {announcement.body_html && (
          /* Only super admins can write this table (RLS), so the HTML comes
             ...
             @tailwindcss/typography is in package.json but never registered with
             an `@plugin` directive in global.css, so under Tailwind v4 the prose
...
export function AnnouncementCarousel({ className }: { className?: string }) {
  ...
  if (count === 0) {
    ...
    return (
      <div className={cn('flex items-center justify-center rounded-xl border border-dashed', className)}>
    ...
    <>
      <div
        className={cn(
          ...
          'relative isolate flex min-h-[260px] flex-col overflow-hidden rounded-xl bg-gradient-to-br from-chart-3 via-primary to-chart-5 p-5 text-white shadow-sm',
          className
        )}
