 ...
 *
 * Costs nothing to fetch: the dashboard already calls useDashboardKPIs on a 60s
 * interval and used only `fleetUtilization`, throwing `overdue`, `dueToday` and
 * `finesOpen` away on every poll. React Query dedupes on the key, so this reads
 * the same response rather than issuing a second request.
 */
