...
/**
 * Aceternity UI "Cloud Shader", added verbatim from the registry
 * (`@aceternity/cloud-shader`, author Manu Arora). Written to disk directly
 * rather than via `npx shadcn add`: the registry entry declares no npm
 * dependencies — it is raw WebGL and `cn` — and the CLI would have rewritten
 * components.json for a file it did not need to fetch.
 *
 * Note for callers: `parseHex` below takes hex or `rgb()`. An `hsl()` string
 * silently parses as RGB — `hsl(244 76% 26%)` becomes rgb(244, 76, 26), an
 * orange, with no error anywhere. Pass hex. `brandSky` in lib/brand-surface
