...
export function IdentityVerificationTab({ customerId }: IdentityVerificationTabProps) {
  ...
  //
  // NOTE: `integration_veriff` was dropped from the Tenant type when the Veriff
  // integration was retired (f1018f6f), which landed while this file was off
  // main. The read is kept (rather than the branch rewritten) so the restore
  // stays faithful — this component has no importer, so nothing renders it and
  // the branch is inert. Whoever owns identity verification should either
  // delete this component or port it to the AI-only flow.
  const verificationMode =
    (tenant as unknown as { integration_veriff?: boolean } | null | undefined)
      ?.integration_veriff !== false
      ? 'veriff'
      : 'ai';
      ...
      <CardContent>
        {verifications.length === 0 ? (
          <EmptyState
            ...
            /* EmptyState swapped its `action` ReactNode for `actionLabel` +
               `onAction` while this file was off main. Ported to the current
               contract; the only behaviour lost is the disabled-while-creating
               state, which the new component does not expose. */
            actionLabel="Start Verification"
            onAction={handleCreateVerification}
          />
        ) : (
