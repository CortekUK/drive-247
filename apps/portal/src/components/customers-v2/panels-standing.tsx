...
/* ─────────────────────────────────────────────────────────────────────────────
 * Customer record v2 — STANDING.
 *
 * The second band: what has been DECIDED about this person. Verification is
 * produced from the first band, so it can fall behind it and says so itself.
 * Account and Consent are decisions in their own right — they cannot be stale,
 * but they are the two panels where a wrong click costs the most, so both are
 * built to make the blast radius impossible to misread.
 * ────────────────────────────────────────────────────────────────────────── */
...
import type { GlobalBlock, PanelProps } from "./types";
...
export function VerificationPanel({ c, onJump, canEdit, drift }: PanelProps & { drift: Drift[] }) {
...
export function AccountPanel({ c, set, onJump, canEdit }: PanelProps) {
  const { tenant } = useTenant();
  const queryClient = useQueryClient();
...
export function ConsentPanel({ c, set, onJump, canEdit }: PanelProps) {
  return (
    <Panel
      title="Consent"
