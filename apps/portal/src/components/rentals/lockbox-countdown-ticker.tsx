...
export function LockboxCountdownTicker({ rentalId }: LockboxCountdownTickerProps) {
  ...
  const offsetMinutes = rentalSettings?.lockbox_send_offset_minutes ?? null;
  ...
  // No auto-send configured or no rental data
  ...
  // STATE: Past due — waiting for cron
  if (msRemaining <= 0) {
    return (
  ...
  // STATE: Counting down
  const isUrgent = msRemaining < 5 * 60 * 1000;
  const countdown = formatCountdown(msRemaining);

  return (
    <div className={cn(
      "flex items-center gap-3 px-4 py-3 rounded-xl border",
      isUrgent ? "bg-amber-500/10 border-amber-500/20" : "bg-primary/5 border-primary/20"
    )}>
      <div className={cn(
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <p className={cn(
            ...
            Lockbox code sends in {countdown}
          </p>
