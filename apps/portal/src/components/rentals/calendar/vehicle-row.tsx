...
interface VehicleRowProps {
  ...
  /** Open the block dialog for this vehicle with no drag (type-in dates). */
  onAddBlock?: (vehicleId: string) => void;
    ...
    >
      ...
      <div className="group/veh sticky left-0 z-10 w-[160px] min-w-[160px] sm:w-[240px] sm:min-w-[240px] border-r bg-inherit px-2 sm:px-3 py-2 flex items-center gap-2 sm:gap-3">
        ...
        {onAddBlock && (
          <button
            type="button"
            title="Block dates for this vehicle"
            onClick={(e) => {
              e.stopPropagation();
              onAddBlock(vehicle.id);
            }}
            className="shrink-0 rounded-md border border-border bg-background text-muted-foreground hover:text-foreground hover:bg-accent p-1.5 opacity-0 group-hover/veh:opacity-100 focus:opacity-100 transition-opacity"
          >
