...
interface RentalProgressOverlayProps {
  isVisible: boolean;
  currentStep: number;
  steps: { label: string }[];
}

export function RentalProgressOverlay({
  isVisible,
  currentStep,
  steps,
}: RentalProgressOverlayProps) {
    ...
    >
      <div className="w-full max-w-md mx-4 p-8 rounded-2xl bg-card border border-border shadow-2xl">
        ...
        <div className="relative h-2 rounded-full bg-muted overflow-hidden mb-2">
          <div
            className="rental-progress-bar-fill h-full rounded-full transition-all duration-700 ease-out"
            style={{ width: `${percent}%` }}
          />
        </div>
