...
function StatementContent({
  ...
  const totals = useMemo(() => {
    let charged = 0, chargedRate = 0, chargedTax = 0, chargedFee = 0;
    ...
    return {
      charged, chargedRate, chargedTax, chargedFee,
      collected, refunded, netReceived,
  ...
  return (
    <div style={containerStyle} className={printable ? "" : "space-y-6"}>
      ...
      >
        <div>
          <h3 className={printable ? "" : "font-semibold mb-2"} style={printable ? { fontWeight: 600, marginBottom: 8 } : undefined}>Bill To:</h3>
        ...
        <div className={printable ? "" : "sm:text-right"} style={printable ? { textAlign: "right" } : undefined}>
          <h3 className={printable ? "" : "font-semibold mb-2"} style={printable ? { fontWeight: 600, marginBottom: 8 } : undefined}>Statement Details:</h3>
          <div className={printable ? "" : "text-sm space-y-1"} style={printable ? { fontSize: 14 } : undefined}>
      ...
      >
        <h3 className={printable ? "" : "font-semibold mb-3"} style={printable ? { fontWeight: 600, marginBottom: 12, marginTop: 0 } : undefined}>Rental Information</h3>
        <div
        ...
        >
          <div>
            ...
            {vehicle.reg && (
              <p className={printable ? "" : "text-muted-foreground text-xs"} style={printable ? { color: "#9ca3af", fontSize: 12, margin: 0 } : undefined}>Reg: {vehicle.reg}</p>
            )}
          </div>
          <div>
            <p className={printable ? "" : "text-muted-foreground"} style={printable ? { color: "#6b7280", margin: 0 } : undefined}>Rental Period</p>
            <p className={printable ? "" : "font-medium"} style={printable ? { fontWeight: 500, margin: 0 } : undefined}>{periodStart} → {periodEnd}</p>
          </div>
          <div>
            <p className={printable ? "" : "text-muted-foreground"} style={printable ? { color: "#6b7280", margin: 0 } : undefined}>Daily Rate</p>
            <p className={printable ? "" : "font-medium"} style={printable ? { fontWeight: 500, margin: 0 } : undefined}>{fmt(data.dailyRate)} / day</p>
          </div>
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
          <tbody>
            ...
            <tr style={{ borderTop: "1px solid #e5e7eb" }}>
              ...
              <td style={{ padding: "10px 14px", textAlign: "right", fontVariantNumeric: "tabular-nums", fontWeight: 500, color: "#16a34a" }}>{fmt(totals.collected)}</td>
            </tr>
            {totals.refunded > 0 && (
              <tr style={{ borderTop: "1px solid #e5e7eb" }}>
                <td style={{ padding: "10px 14px" }}>Total Refunded</td>
                <td style={{ padding: "10px 14px", textAlign: "right", fontVariantNumeric: "tabular-nums", fontWeight: 500, color: "#dc2626" }}>-{fmt(totals.refunded)}</td>
              </tr>
            )}
      ...
      >
        <p style={printable ? { margin: 0 } : undefined}>
          This is a Pay-As-You-Go statement showing all daily charges accrued under this rental.
        </p>
...
function StatusLabel({ inv, printable }: { inv: PaygInvoiceRow; printable: boolean }) {
  ...
  return printable
    ? <span style={{ color: "#d97706", fontWeight: 500 }}>Open</span>
...
export function PaygStatementDialog({
  ...
  const companyName = tenant?.company_name || "Statement";
  const accentColor = "#6366f1"; // portal design system indigo accent
