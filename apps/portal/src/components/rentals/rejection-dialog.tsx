...
export default function RejectionDialog({
  ...
  const totalRefundAmount = regularPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (paymentsProcessed > 0) {
        ...
        if (regularRefunded > 0) {
          description += ` ${formatCurrency(regularRefunded, currencyCode)} refunded`;
        }
        if (manualRequired > 0) {
          ...
          <TabsContent value="email" className="space-y-4 py-4">
            ...
            <Alert>
              ...
              <AlertDescription className="text-sm">
                ...
                {totalRefundAmount > 0 ? (
                  <>
                    {regularPayments.length} payment(s) totalling <strong>{formatCurrency(totalRefundAmount, currencyCode)}</strong> will be refunded.
                  </>
                ) : (
