...
function Composer({
  ...
  const submit = async () => {
    ...
    try {
      ...
      // `tenant_notes` does not exist yet (ops/tenant_notes.sql is unapplied),
      // and for THAT case "try again" is a lie — retrying can never succeed, so
      // the operator retypes the note, loses it again, and concludes the
      // product is broken rather than unfinished.
      setError(
        err instanceof Error && err.message === NOTES_UNAVAILABLE
          ? 'Notes are not switched on yet. Nothing you write here can be saved.'
          : 'Couldn’t save that. Your note is still here — try again.'
      );
    } finally {
      setSaving(false);
    }
...
export function RemindersCard() {
  ...
  /**
   * A REFUSED TICK OR DELETE HAS TO SAY SO.
   *
   * Both mutations are optimistic: the cache moves first and `onError` in
   * `use-tenant-notes.ts` puts it back. Rollback alone is a silent failure — the
   * row un-ticks itself half a second after the operator ticked it, or the
   * deleted note reappears, and the only story that fits is "I mis-clicked". So
   * they click again, and again, against a write that is being refused every
   * time.
   *
   * This is the same treatment the composer already gives a refused ADD: say it
   * plainly, and say what state their work is in now. The hook keeps throwing
   * (that asymmetry is its contract — reads swallow, writes do not), the card
   * catches. `mutateAsync` rather than `mutate` is what makes the rejection
   * reachable here at all; `mutate` swallows it and there is nothing to catch.
   */
  const [actionError, setActionError] = useState<string | null>(null);

  const run = async (action: () => Promise<void>, failure: string) => {
    // Cleared on every attempt, so a stale message can never sit over a write
  ...
  return (
    <Card
      ...
      // here: a long list of notes come to roughly double 352px, so
    ...
    >
      {actionError && (
        ...
        // operator has already made and may have moved on from — it announces
        // on insertion rather than waiting to be tabbed into.
        <div
          role="alert"
          // `shrink-0` so a two-line message is never squeezed into one and
          // clipped when the card is at its minimum height — the list below it
          // is the flexible part, and it already scrolls.
          className="flex shrink-0 items-start gap-2 border-b border-[var(--pv-line)] bg-[var(--pv-wash)] px-5 py-2"
        >
          <p className="min-w-0 flex-1 text-[11px] leading-tight text-[var(--pv-late)]">
            {actionError}
          </p>
          <button
            type="button"
            ...
            className="mt-px shrink-0 text-[var(--pv-ink-3)] transition-colors hover:text-[var(--pv-ink-2)]"
          >
            <X className="size-3" strokeWidth={2.5} />
          </button>
        </div>
          ...
          <div className="divide-y divide-[var(--pv-line)]">
            {notes.map((note) => (
              <NoteRow
                ...
                note={note}
                now={now}
                // The copy names where their work ended up, not what went
                // wrong: after a rollback the tick is undone and the note is
                // still there, and that is the only part the operator can act
                // on. Mirrors the composer's "Your note is still here".
                onToggle={(n) =>
                  void run(
                    () => toggleDone.mutateAsync(n),
                    'Couldn’t save that. The note is back the way it was — try again.',
                  )
                }
                onDelete={(n) =>
                  void run(
                    () => deleteNote.mutateAsync(n),
                    'Couldn’t delete that. The note is still here — try again.',
                  )
                }
              />
            ))}
