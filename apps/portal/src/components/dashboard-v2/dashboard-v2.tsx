...
import { MoneyAtRisk } from "@/components/dashboard-v2/money-at-risk";
import { OnTheMoveToday } from "@/components/dashboard-v2/on-the-move-today";
 ...
 *
 * COLOUR: the chart ramp reads `--chart-1..5`, which are defined in
 * `styles/v2-theme.css` under `.v2-theme`. The root layout puts that class on
 * <body> for tenants gated into the `theme` area, and `theme` and `dashboard`
 * carry the same tenant list in `lib/v2.ts` — so wherever this component
 * renders, those tokens are present.
 *
 * TENANT ISOLATION: the one query in this file (`dashboard-analytics`) filters
 * every leg by `.eq('tenant_id', tenant.id)`. RLS is OFF on `rentals`
 * (V2_PLAN §5); that filter is the only thing separating operators.
 */
