...
function SheetShell({
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      {/* The repo carries two copies of @types/react (portal pins one, the root
          resolves another), so radix's ReactNode and ours are nominally
          different types. Same value, different declaration — narrow the node
          to the one SheetTrigger declares rather than leak a second error class
          in here. */}
      <SheetTrigger asChild>
        {trigger as ComponentProps<typeof SheetTrigger>["children"]}
      </SheetTrigger>
      {/* `flex flex-col` is load-bearing: the body below is `min-h-0 flex-1`,
          and the shared SheetContent is a plain block. `gap-0 p-0` override the
          primitive's own spacing (tailwind-merge, last class wins). `pr-12` on
          the header keeps "View all" clear of the sheet's built-in close X. */}
      <SheetContent side="right" className="flex flex-col gap-0 p-0">
        <SheetHeader className="h-16 flex-row items-center justify-between space-y-0 border-b px-5 pr-12">
...
function MessagesList({ onClose }: { onClose: () => void }) {
  // Tenant isolation is the hook's: `useChatChannels` filters on
  // `.eq('tenant_id', tenant.id)` and keys its cache on the tenant. Nothing
  // here queries Supabase directly, so there is no filter to forget.
