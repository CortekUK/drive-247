...
const STEPS: readonly TourStep[] = [
  ...
  },
  {
    ...
    body: 'This reads your real hours and your real blocked dates, but every change you make on it is a sketch. Reload the page and it is gone.',
    route: '/blocked-dates',
  ...
  },
  {
    ...
    body: 'Seven cells, one per weekday, each sitting directly above the dates it governs. Change Monday here and every Monday below it changes with it.',
    route: '/blocked-dates',
    anchors: ['[data-tour="availability-pattern"]', ...LIST_HEADING],
    // `top`: the strip is at the head of the card and the calendar it governs
    // is underneath, so a card below would cover the thing the body points at.
    side: 'top',
    notes: [
      {
  ...
  },
  {
    ...
    label: 'The tally',
    title: 'How the week adds up',
    body: 'Days open, days closed, and how many dates you have changed by hand. All three count the week on screen rather than your whole calendar.',
    route: '/blocked-dates',
    anchors: ['[data-tour="availability-counts"]', CONTROLS, ...LIST_HEADING],
    side: 'bottom',
    notes: [
      {
        // The only undo on this screen, and it is all-or-nothing. Anchored so
        // it is not promised to a header that does not carry the button.
        text: 'Reset preview, beside it, throws the whole sketch away and puts your real hours back.',
        anchors: ['[data-tour="availability-reset"]'],
      },
    ],
  },
  {
