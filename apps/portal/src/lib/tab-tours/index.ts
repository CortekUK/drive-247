 ...
 */
export const TAB_TOUR_IDS = ['customers', 'vehicles', 'rentals', 'payments'] as const;
...
export function tabTourVariant(id: TabTourId): TabTourVariant {
  ...
  if (id === 'payments') return 'full';
  const ids = readSampleIds();
  const has =
    id === 'vehicles' ? ids.vehicleId : id === 'customers' ? ids.customerId : ids.rentalId;
  return has ? 'full' : 'empty';
}
