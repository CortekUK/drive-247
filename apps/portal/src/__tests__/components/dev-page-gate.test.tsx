...
describe('Developer page — shows', () => {
  it('renders for northwind on localhost in a development build, with exactly the three buttons', async () => {
    ...
    expect(button(LANDING_PAGE)).toBeDefined();
    // Three abilities, by request. The count is scoped to the action cards
    ...
    expect(container.querySelectorAll('[data-dev-action]')).toHaveLength(3);
    expect(container.querySelectorAll('[data-dev-action] button')).toHaveLength(3);
  });
...
describe('Developer page — "Start from the landing page"', () => {
  // The fallback the button navigates to when NEXT_PUBLIC_WEB_BASE_URL is
  // unset, exactly as it is in this suite — apps/web's dev port in this
  // worktree (booking 4001, portal 4002, web 4003, admin 4004, bonzah 4005).
  const DEMO_SIGNUP_URL = 'http://localhost:4003/demo-signup';

  it('resets the same state as "Start as a first-time operator", then navigates to the demo signup journey — not the dashboard', async () => {
    currentTenant = NORTHWIND;
    deleteRows = [{ id: 'row-1' }];
    ...
    await render();
    await click(button(LANDING_PAGE));

    expect(supabaseCalls).toEqual(['delete tenant_first_run tenant-northwind']);
    expect(localStorage.getItem(tourSeenKey('app-user-1'))).toBeNull();
    expect(localStorage.getItem('setup-guide-state-tenant-northwind')).toBeNull();
    // The other button's whole point: this one goes to the public journey,
    // never straight to `/`.
    expect(assign).toHaveBeenCalledTimes(1);
    expect(assign).toHaveBeenCalledWith(DEMO_SIGNUP_URL);
    expect(container.querySelector('[data-dev-status="error"]')).toBeNull();
  });

  it('THE RLS CASE — never navigates, so the operator is never sent off to re-arm a wizard that stayed dark', async () => {
    currentTenant = NORTHWIND;
    // The DELETE "succeeds" with zero rows, and the row is still readable —
    // RLS blocked it.
    deleteRows = [];
    ...
    // The reset failed, so this button must not send the operator anywhere —
    // racing the navigation ahead of a failed reset is exactly the bug this
