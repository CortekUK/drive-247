...
vi.mock("@/components/settings/appearance/logos-v2", () => ({
  ...
  LogosV2: ({ faviconUrl, logoUrl, tabTitle, brandColor, portalName, onFaviconChange, onLogoChange }: any) => (
    <div
...
describe("Portal name: a live preview of where the name shows", () => {
  it("shows the name in the sidebar row and the browser tab, and follows the typing", () => {
    ...
    // No square icon yet: the sidebar badge falls back to the full logo, the tab to the platform icon.
    expect(within(preview).getByAltText("Your logo in the sidebar")).toHaveAttribute("src", "https://cdn.test/logo.png");
  ...
  it("the preview on its own: the square icon goes in the tab, the badge takes what the sidebar would", () => {
    render(
      <PortalNamePreview
        name="Northwind"
        tabIconUrl="https://cdn.test/icon.png"
        sidebarIconUrl="https://cdn.test/icon.png"
    ...
    expect(screen.getByAltText("Your logo in the sidebar").className).toContain("h-8 w-8");
  });
  ...
  it("gives the name the sidebar's own room: the border sits outside the 256px, not inside it", () => {
    ...
    render(
      <PortalNamePreview name="Northwind" tabIconUrl={null} sidebarIconUrl={null} tabTitle="Northwind - Portal" />,
    );
