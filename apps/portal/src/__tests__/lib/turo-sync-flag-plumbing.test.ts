 ...
 * session exists, so one ungranted column in TENANT_MINIMAL_COLUMNS strips branding
 ...
 * keeps TENANT_MINIMAL_COLUMNS, so a core column has no rung below it. Hence the
 ...
 * TENANT_MINIMAL_COLUMNS against information_schema and reports every entry anon
 ...
 * copy is the exact bug under test. Edit TENANT_MINIMAL_COLUMNS and this suite
...
const CORE = liftColumnList('TENANT_MINIMAL_COLUMNS');
...
describe('Turo Sync flag — column-list plumbing (TenantContext.tsx)', () => {
  it(`keeps ${FLAG} in TENANT_OPTIONAL_COLUMNS and OUT of TENANT_MINIMAL_COLUMNS`, () => {
    const why =
      ...
      'TENANT_MINIMAL_COLUMNS looks like tidying up. It is not.\n' +
      '\n  * The 42501 retry ladder in TenantContext.tsx keeps TENANT_MINIMAL_COLUMNS on EVERY ' +
    ...
    expect(coreColumns, `${FLAG} was promoted into TENANT_MINIMAL_COLUMNS.${why}`).not.toContain(FLAG);
  });
  ...
  it('keeps the three column lists pairwise disjoint', () => {
    ...
    expect(
      ...
      'Column(s) in BOTH TENANT_MINIMAL_COLUMNS and TENANT_INSHUR_COLUMNS. They are concatenated ' +
    ...
    expect(
      ...
      'Column(s) in BOTH TENANT_MINIMAL_COLUMNS and TENANT_OPTIONAL_COLUMNS. Every rung of the ' +
  ...
  it('has no duplicate column inside any one list', () => {
    ...
    expect(dupes(coreColumns), 'TENANT_MINIMAL_COLUMNS names a column twice.').toEqual([]);
  ...
  it('spells every column in all three lists as a bare identifier', () => {
    ...
    const named: Array<[string, string[]]> = [
      ['TENANT_MINIMAL_COLUMNS', coreColumns],
...
describe(`Turo Sync flag — DEPLOY GATE snapshot (${SQL_PATH})`, () => {
   ...
   *
   * The DEPLOY GATE is a hand-pasted COPY of TENANT_MINIMAL_COLUMNS, and its whole
   ...
   * Nobody reviewing a one-line change to TENANT_MINIMAL_COLUMNS thinks to open a
   * .sql file in a PoC directory. This test is what makes them.
   */
  it('matches TENANT_MINIMAL_COLUMNS exactly — same entries, same order', () => {
    ...
    const remedy =
      ...
      `TENANT_MINIMAL_COLUMNS from ${TENANT_CONTEXT_PATH}, verbatim.\n` +
      '  3. Re-run the DEPLOY GATE against production. It must return ZERO rows before the ' +
      'change to TENANT_MINIMAL_COLUMNS ships — any row it returns is a tenant-wide branding and ' +
      'login outage waiting on your deploy.\n' +
      '\nOnly genuine TENANT_MINIMAL_COLUMNS entries belong in the snapshot. The INSHUR columns and ' +
    ...
    expect(
      snapshot,
      `The DEPLOY GATE snapshot has drifted from TENANT_MINIMAL_COLUMNS (snapshot ` +
        `${snapshot.length} entries, real list ${coreColumns.length}).${remedy}`,
    ).toEqual(coreColumns);
  });

  it('states the core-column count that TENANT_MINIMAL_COLUMNS actually has', () => {
    ...
    expect(
      ...
      `${SQL_PATH} claims ${claim![1]} core columns were verified as granted, but ` +
        `TENANT_MINIMAL_COLUMNS now has ${coreColumns.length}. Update the number AND re-run the ` +
  ...
  it('grants anon nothing that is not a real column in one of the three lists', () => {
    ...
    expect(
      ...
      `${SQL_PATH} grants anon SELECT on ${JSON.stringify(unknown)}, which appear in none of ` +
        'TENANT_MINIMAL_COLUMNS / TENANT_INSHUR_COLUMNS / TENANT_OPTIONAL_COLUMNS. Either the ' +
