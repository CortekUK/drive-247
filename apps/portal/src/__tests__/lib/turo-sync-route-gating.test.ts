...
const coreColumns = columnList('TENANT_MINIMAL_COLUMNS');
...
describe('gate 2: the Turo Sync page guard', () => {
  ...
  it('WRAPS the screen rather than early-returning inside it', () => {
    ...
    // The guard may consult the CANARY GATE and the tenant, and nothing else.
    //
    // `useV2` was added when this feature was grafted onto main: five tenants
    // already carry turo_bridge_enabled = true from the PoC, two of them live
    // operators, so the tenant flag alone would have opened the screen to them
    // the moment it deployed. It is a context read, not a query — it mounts no
    // data hooks, so the reason this guard wraps the screen is unchanged.
    //
    // Order matters and is asserted: the canary gate is consulted BEFORE the
    // tenant flag, because the two refusals say different things and only one
    // of them is true for a tenant who is not on the rollout.
    expect(hookCallsIn(defaultExport as ts.Node)).toEqual(['useV2', 'useTenant']);
...
describe('gate 2: the flag column is spelled and compared identically everywhere', () => {
  it('is carried in TENANT_OPTIONAL_COLUMNS and kept out of TENANT_MINIMAL_COLUMNS', () => {
