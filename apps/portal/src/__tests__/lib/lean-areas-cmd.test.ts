...
describe("CMD gate call sites", () => {
  ...
  it("does NOT filter the CheckMyDriver card off the Integrations board", () => {
    // The board used to hide CMD from the canary. That was reversed when CMD
    // gained a "coming soon" panel that connects nothing: a preview card
    // exposes no feature, so the lean gate has nothing to hide on this screen.
    // This assertion pins the reversal so a future "tidy-up" cannot quietly
    // put the filter back — and so a delete cannot take the card from the
    // other 56 tenants, which is the outage this whole file exists to prevent.
    const src = read("app/(dashboard)/integrations/integrations-board.tsx");
    expect(src).not.toMatch(/isAreaHidden\("cmd"/);
    expect(src).not.toMatch(/i\.name !== "CheckMyDriver"/);
    // The grid still renders the list it is given. Since pinning landed, the
    // list reaches the screen through `partitionByPins` — which SPLITS it into
    // pinned and the rest and re-orders them, but never drops one — and both
    // halves are rendered. Pinning both halves here is what keeps a future
    // filter from hiding inside the partition.
    expect(src).toMatch(/partitionByPins\(visibleIntegrations, pinned\)/);
    expect(src).toMatch(/\{pinnedCards\.map\(\(it\) => [({]/);
    expect(src).toMatch(/\{rest\.map\(\(it\) => [({]/);
    // …and the entry itself is on main for everyone.
    expect(src).toMatch(/name: "CheckMyDriver"/);
    // …with a panel behind it, so the card opens a real dialog for every tenant.
    const registry = read("app/(dashboard)/integrations/_panels/registry.ts");
    expect(registry).toMatch(/CheckMyDriver: \{ Panel: CheckMyDriverPanel/);
  });
