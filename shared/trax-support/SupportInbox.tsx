...
/** The ticket list and one ticket's conversation: the portal's Support section and the
 * admin support inbox. Both are full-width sections, so the list sits beside the thread
...
export function SupportInbox({call,admin=false,initialId,compose,scope}:{call:MessagingCall;admin?:boolean;initialId?:string;compose?:SupportCompose;scope:string}){
  ...
  const beginNew=()=>{let value=crypto.randomUUID();try{value=sessionStorage.getItem(nonceKey)||value;sessionStorage.setItem(nonceKey,value);}catch{}setNonce(value);setCreating(true);setId(null);setSubject(compose?.summary??'');setDraft('');outgoing.current=null;setNotice('');};
  const send=async()=>{
    ...
    <div className="flex min-h-0 flex-1 flex-col md:flex-row">
      ...
      <section className={'flex min-h-0 min-w-0 flex-1 flex-col '+(!creating&&!id?'hidden md:flex':'')} aria-label="Support conversation">
        ...
        {(creating||thread)&&<form className="mt-auto shrink-0 space-y-2 border-t border-border p-4" onSubmit={e=>{e.preventDefault();void send();}}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2">{creating&&<button type="button" className={button} disabled={busy} onClick={()=>setCreating(false)}>Cancel</button>}{admin&&thread&&<select aria-label="Status after sending reply" value={status} disabled={busy||!!outgoing.current} onChange={e=>setStatus(e.target.value)} className="rounded-md border border-input bg-background p-2 text-sm"><option value="">Keep status</option>{Object.entries(statusLabel).map(([key,label])=><option key={key} value={key}>{label}</option>)}</select>}<span role="status" className="text-xs text-muted-foreground">{busy?'Sending…':notice}</span></div><button className={primary} disabled={busy||!draft.trim()||(creating&&!subject.trim())}>{busy?'Sending…':outgoing.current?'Retry send':'Send'}</button></div>
        </form>}
      </section>
    </div>
  </div>;
