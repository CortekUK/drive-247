...
import { handleMessaging, type MessagingDatabase } from '../../supabase/functions/trax-support/support/messaging';
import { createSupportReads, type SupportDatabase } from '../../supabase/functions/trax-support/support/reads';
import { traxTestTenantSlugs } from '../../apps/portal/src/lib/trax-test-tenants';
/** Same server handler in both local apps. No secret ever reaches a browser. */
export async function localMessaging(request:Request){
  ...
  return handleMessaging(request,{reads:createSupportReads(db as unknown as SupportDatabase),db:db as unknown as MessagingDatabase,enabled:process.env.TRAX_SUPPORT_STORAGE==='enabled',testTenantSlugs:traxTestTenantSlugs()});
}
