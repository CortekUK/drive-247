...
> **This is the current document.** An earlier copy sits at the repository root,
> `TURO_DRIVE247_EXTENSION_IMPLEMENTATION.md`, and is now out of date — it
> predates the sign-in fix and still describes the single blanket error message
> and the password being cleared on failure, both of which were the bug. The
> root copy was left untouched because the sign-in task scoped all edits to
> `turo-bridge-poc/`. Delete it, or replace it with this file, when convenient.
...
9. **`app_users` is readable with the bare anon key and no session.** Found while
   diagnosing the sign-in: a plain `GET /rest/v1/app_users` carrying only the public
   anon key returns staff rows — emails, `tenant_id`, `auth_user_id`, `is_active` — for
   all 77 rows across every tenant. This is **a backend RLS gap, not an extension bug**,
   it predates this work, and fixing it requires a migration outside
   `turo-bridge-poc/`. It is recorded here because the profile lookup currently
   succeeds partly because of it; the lookup is already written to work correctly under
   proper RLS (it reads as the user, filtered by `auth_user_id`), so tightening the
   policy should not break it — but that must be verified when the policy lands.
10. **One timing-sensitive test can flake under load.** `background-orchestrator`'s
...
7. **Close the `app_users` RLS gap** (see §14.9). The public anon key currently reads
   every tenant's staff list. This is outside `turo-bridge-poc/` and needs a migration;
   re-run the extension's sign-in afterwards to confirm the profile lookup still passes
   under the tightened policy.
8. **Audit `booking_v2_enabled`** — unrelated to this feature, but it has silently
