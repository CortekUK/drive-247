...
-- literal TENANT_MINIMAL_COLUMNS string out of
...
--
-- When TENANT_MINIMAL_COLUMNS changes, paste the new value in below -- it is a
...
-- Only genuine TENANT_MINIMAL_COLUMNS entries belong in the snapshot; verified
-- 2026-09-12 that all 8 of them are granted, i.e. there is no second
-- customer_theme_mode lurking today.)
--
-- WHY THIS LIST SHRANK FROM 57 TO 8, on the graft onto main.
-- The constant this snapshots was renamed and, more importantly, RESPLIT:
-- TENANT_CORE_COLUMNS became TENANT_MINIMAL_COLUMNS (8) plus
-- TENANT_OPTIONAL_COLUMNS (the other ~49). Only the minimal tier is kept on
-- EVERY rung of the 42501 retry ladder, so only it can take a tenant's whole
-- row down. The ~49 that moved to OPTIONAL are shed by the ladder exactly the
-- way turo_bridge_enabled is, so listing them here would make this gate cry
-- wolf about columns whose design is to be sheddable. Fewer rows, same rule.
-- ============================================================================
WITH core(col) AS (
