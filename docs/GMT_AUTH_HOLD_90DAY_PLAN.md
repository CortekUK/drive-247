...
- **Verified first-hand against the Visa Core Rules and Visa Product and Service Rules, 18 April 2026 edition** (46,281 lines, downloaded and grepped): **Table 5-12** caps Estimated Authorizations for Vehicle Rental / Lodging / Cruise Line merchants at **30 calendar days**, and no authorization window anywhere in the rulebook exceeds 30. Nothing in the rulebook prohibits placing a subsequent authorization, and Visa's separate published merchant guidance on estimated/incremental authorizations describes closing the original within its validity period and requesting a new one for extended rentals.
  > **CORRECTION (2026-08-09):** an earlier draft of this document quoted Visa as saying such authorizations *"need to be reversed and a new authorization must be requested."* That exact wording is **NOT in the Core Rules** — a grep for `reversed and a new` and `must be requested` returns **zero hits**. The substance holds (the secondary guidance says the original must be closed within its validity period and a new authorization **may** be requested), but do not cite it as a verbatim Core Rules quote.
...
|---|---|---|
| ~~Is the pg_cron job live?~~ **ANSWERED 2026-08-09 — YES.** | Queried prod directly. | `refresh-deposit-holds` is **jobid 57, schedule `0 3 * * *`, active = true**, with **56 runs, all status `succeeded`**, most recent `2026-08-09 03:00:00 UTC`. The "it was silently never scheduled again" hypothesis (commit `9ab190f2`, 2026-06-15) is **DISPROVEN — do not repeat it.** No migration schedules it because it was created out-of-band, which is why the repo's only trace is `sim-control/cron-manifest.json`. **Caveat that still stands:** `job_run_details.status='succeeded'` only means pg_net dispatched the HTTP request — it says nothing about what the function did. So the diagnosis is "our refresh runs but is fragile", not "we have never refreshed anything". Prod hold book at that date: of GMT's 10 Active rentals, 2 `held`, 1 `expired` on a LIVE rental, 6 `NULL` (4 with a stale past `deposit_hold_expires_at` — the fingerprint of failed placement), 1 `released`. |
