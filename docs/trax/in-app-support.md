...
The model is not involved in My Tickets, reads, replies, unread counts or direct ticket creation. The escalation composer uses the existing authenticated issue handler only to copy its authorized context. During an AI outage the independent My Tickets composer remains available. Credentials, payment/card identifiers and obvious personal contact data are redacted before persistence. Human messages do not authorize rental, payment, insurance or other business writes.
