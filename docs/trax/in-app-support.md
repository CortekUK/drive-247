...
Human support lives in the portal's own **Support** section (`/support`), not inside TRAX. The profile menu's Support item and TRAX's Open Support control both go there, so both entry points show the same authorized tickets; TRAX closes behind the navigation and keeps its AI conversation and unsent draft. Support lists the tenant's tickets, opens one to read its messages and reply, and New request opens a subject/message composer. The route is gated on the same `chrome` V2 rollout the endpoints enforce, and no tenant is sent to the platform (super-admin) inbox. For an escalated issue, Communicate with Support opens Support at `/support?issue=<id>` with that composer, the issue summary available for editing and the authorized redacted troubleshooting context still attached through the open TRAX conversation; an issue that already has a ticket opens `/support?ticket=<id>`. If the issue is no longer in the conversation, the composer stays open as a plain new request and says so. Opening Support, the composer, or cancelling creates no ticket. The first non-empty Send commits the ticket, initial message, copied troubleshooting handoff and email job. A successful response shows the actual reference; retrying does not create a duplicate. An issue with a ticket opens its existing thread.
