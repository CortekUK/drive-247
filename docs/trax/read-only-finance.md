...
Activation requires a restricted Stripe read key (`TRAX_STRIPE_READ_{UK|UAE}_{LIVE|TEST}_KEY`, `rk_` only) and the deployed `trax-stripe-read` edge function; `TRAX_FINANCE_READS=disabled` turns it off even when a key exists, and `=enabled` is retained for environments that provide the key some other way. Who may use payment checks comes from Supabase (staff role and manager tab permissions); there is no separate grant list. The platform secrets are never used (see the configuration section below).
...
Payment access follows the authenticated staff record in Supabase, the same finance staff policy as guidance: head admins and admins (super-admins act as head admin) get rental payments and account funds; managers need the Payments tab permission, plus Rentals for rental payments; ops and viewers get none. Nothing is available until a restricted Stripe read key is configured.

1. A configured restricted read key enables configuration loading on the V2 server only. `TRAX_FINANCE_READS=disabled` suppresses it regardless; `=enabled` forces loading without one, for an environment that supplies the key elsewhere — every read still refuses with `stripe_configuration_required` until a key resolves.
...
4. Stripe keys: the adapter reads `TRAX_STRIPE_READ_{UK|UAE}_{LIVE|TEST}_KEY` by name, accepts only a value beginning `rk_<mode>_`, and matches platform and mode exactly. It has **no fallback to the platform secrets** (`STRIPE_LIVE_SECRET_KEY` and friends): those exist on the deployed project and can write, so a fallback would have meant authenticating to live Stripe with a full-access key by default, leaving least privilege an accident of this module's contents rather than a property of the credential. Without a restricted key every read refuses with `stripe_configuration_required` and no request is sent. A backend outside Supabase (the local portal) calls the `trax-stripe-read` edge function, which requires the service-role credential and runs the same fixed read-only GETs; key values never leave Supabase. Restricted keys need only the read permissions for Balance, PaymentIntents, Charges and Checkout Sessions.
