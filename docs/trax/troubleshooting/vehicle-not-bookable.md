...
Phase 1 can explain this and open an authorized record. It cannot say which rental blocks the vehicle, that keys physically returned, that a hold was released, or that the car is now available. Its response must say no live check was performed.

Before Phase 2, verify deployed check_rental_overlap and the channel-specific differences documented in ../workflows/vehicle-availability.md. Do not run a trial booking INSERT or mutating health evaluator as a diagnostic.
