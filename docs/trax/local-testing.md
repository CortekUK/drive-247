...
Attachments need the `trax-support-attachments` private bucket and the attachments migration in the configured project; without them the paperclip is not offered and messages still send. For the Support section, run `node tests/trax/support-browser.mjs` (add `--manual` to drive it by hand). It mounts the real `PortalSupport` over a scripted in-memory messaging API and checks the redesign: one page header, a ~320px ticket list, no outer page scroll, content-sized bubbles that wrap long text, date separators and grouped turns, a header and composer that stay on screen while the history scrolls, no jump to the bottom while reading older messages, the Issue details disclosure, a failed send that keeps the draft and retries once, a screenshot that is attached, sent and shown in the conversation, a file type the conversation refuses, empty filter results, a ticket created only by the first Send, and the phone's list/conversation switch with Back. Screenshots are in `artifacts/trax-support-ui`.
