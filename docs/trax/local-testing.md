...
For the new support workflow, run `node tests/trax/browser.mjs --support --manual`. It uses the real V2 UI/hook/handler with isolated storage fixtures. Ask about a missing payment: TRAX creates the ticket itself and answers with the real fixture reference and an Open support ticket button, which opens that conversation in the Support section. The fixture also covers reuse on a second ask, a failed handoff that invents no reference, its retry, and Retention. References are explicitly fixture-labelled and disappear when this harness stops. Omit `--manual` for automated browser checks. Actual SQL persistence/retention tests and the approved-environment activation sequence are in [support-escalation.md](support-escalation.md).
...
Attachments need the `trax-support-attachments` private bucket and the attachments migration in the configured project; without them the paperclip is not offered and messages still send. For the Support section, run `node tests/trax/support-browser.mjs` (add `--manual` to drive it by hand). It mounts the real `PortalSupport` and the real Support rail inside the v2 sidebar provider over a scripted in-memory messaging API, and checks the three-area workspace: the ticket list in the sidebar's slot with Back to portal and the tenant (no fourth column), no outer page scroll, the conversation between the rail and Details | TRAX Summary, the generated TRAX message drawn as an event while a person's message that mentions TRAX stays a bubble, Details fields and record links with a read-only status, the TRAX summary and its labelled saved conversation with no score and no composer, a slow ticket switch that shows loading rather than the previous ticket, the no-TRAX-conversation empty state, content-sized bubbles in a long history with the composer and panel fixed while it scrolls, no jump while reading older messages, a failed send that keeps the draft and retries once, an attachment sent and shown, a refused file type, empty filter results, New ticket from the rail, a collapsed sidebar handing the list back to the page, the Details drawer at 1100px, and the phone's list/conversation switch with Back and the drawer. Screenshots are in `artifacts/trax-support-ui`.

For the sidebar's Support row, run `node tests/trax/sidebar-badge-browser.mjs`. It renders the real v2 sidebar with its data hooks answered from memory and checks that Support sits directly under Fines with the same row, that the unread badge is right-aligned and centred without moving the label as the count goes 0 → 2 → 12, the accessible name, dark mode, the collapsed corner badge and the /support link. Screenshots are in `artifacts/support-sidebar-badge`. The message counting itself — replies counted as messages, status changes excluded, duplicates and reloads stable, hidden tabs not reading, one conversation's reading leaving the other ticket counted — is in `messaging-browser.mjs` below, against real SQL, together with the per-ticket row badges on both sides at once: two support replies show 2 on the tenant's row, three tenant messages show 3 on support's row, TRAX's summary and a requester's own messages, support's own replies and status changes add nothing, a second administrator reading a ticket leaves the first administrator's count, reading one conversation clears only that row, and the counts survive a reload.
...
Start with `node tests/trax/messaging-browser.mjs` from the repository root. It exercises the actual shared inbox/admin sidebar and backend against isolated PostgreSQL with two fixture tenants and one platform support identity; no live email is sent. The platform page is laid out as `/admin/support` is, with the Support rail in the navigation's slot, and a TRAX-escalated ticket is written through the real `trax_support_submit_message` SQL: both sides see the generated message as an event and the same TRAX Summary, support sees the company and the status control in Details, the event is acknowledged as read, switching to a ticket without TRAX context moves all three areas, a manager whose rentals grant was withdrawn keeps the conversation but is told the TRAX context is not available (not that there is none), and at 1100px support's Details is a drawer holding the one status control. It also runs the status-synchronisation check with a tenant page and an authorized platform page open at the same time: the administrator selects In progress and then Resolved, each change is read back from the ticket row, the tenant's badge follows without a refresh and survives a reload, a forced failure leaves the stored status showing, and the tenant is never given the control. Screenshots are in `artifacts/trax-messaging`. For configured local/staging apps, use the migration, grant, worker and scheduler checklist in [in-app-support.md](in-app-support.md). Support tickets stay independent of OpenAI.
