...
For the Support section, run `node tests/trax/support-browser.mjs` (add `--manual` to drive it by hand). It mounts the real `PortalSupport` over a scripted in-memory messaging API and checks the redesign: one page header, a ~320px ticket list, no outer page scroll, content-sized bubbles that wrap long text, date separators and grouped turns, a header and composer that stay on screen while the history scrolls, no jump to the bottom while reading older messages, the Issue details disclosure, a failed send that keeps the draft and retries once, empty filter results, a ticket created only by the first Send, and the phone's list/conversation switch with Back. Screenshots are in `artifacts/trax-support-ui`.
