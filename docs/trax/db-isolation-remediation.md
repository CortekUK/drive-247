...
**One deliberate cross-account read, and why it stays.** `finance-reads.ts:28` (`accountOwners`) counts how many tenants a Stripe account id is mapped to, across all tenants. It returns a number from a `head: true` count — never a row, never an id — and its only effect is to *withhold*: if the answer is not exactly 1, `finance-tools.ts:108` refuses and no balance is requested, because a shared account's funds are not this tenant's money. `trax-finance.test.ts:59` proves the refusal happens without exposing the other account, and `finance-tools.ts:112` re-checks it after the fact so a mapping that changes mid-request fails with `409` instead of attributing someone else's balance. Removing this check would not improve isolation; it would let a shared account be reported as the tenant's own.
