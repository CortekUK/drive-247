...
Customers hold the person/business record linked to rentals. Vehicles hold fleet records and rental configuration. Open Customers or Vehicles and select a record. V2 uses separate customer and vehicle section rails; do not interpret rental workflow headings as customer detail routes. Below the width where a side column fits, a record's rails are not in the sidebar at all — they are buttons on the record's own dock (`components/ui-v2/record-dock.tsx`), and the sidebar goes back to being the navigation.
