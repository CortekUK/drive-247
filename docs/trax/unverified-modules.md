...
| Balance adjustments (canary) | Moving a customer's balance up or down by hand — a wrong charge, money received outside the platform, goodwill — and asking for a payment "for" something, each kept with who, when and why | Canary-only by slug and hidden until its migration is applied; the three operations, undo, off-platform payments and permissions are not deployed or verified end to end |
