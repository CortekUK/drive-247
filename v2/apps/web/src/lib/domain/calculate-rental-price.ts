/**
 *  DUPLICATED FILE — KEEP IN SYNC (THREE COPIES NOW)
 * This pricing engine exists in three places:
 *   apps/booking/src/lib/calculate-rental-price.ts        (v1, customer)
 *   apps/portal/src/lib/calculate-rental-price.ts         (v1, staff)
 *   v2/apps/web/src/lib/domain/calculate-rental-price.ts  (v2, customer)  <- you are here
 * Any change to one MUST be mirrored to the others or the customer (booking /
 * v2 web) and staff (portal) prices will silently disagree.
 * The two v1 copies must remain BYTE-IDENTICAL to each other:
 *   `diff apps/booking/src/lib/calculate-rental-price.ts apps/portal/src/lib/calculate-rental-price.ts`
 * should print nothing. This v2 copy is LOGIC-identical — only this banner
 * differs, because it has to name a third location.
 * Verify with: scripts-v2-guard/check-pricing-parity.sh
