...
/** What leaves the server about an issue.
 *
 * The escalation SCORE and the check counter stay here: they are support policy
 * state, not something a tenant (or the model, which sees this same view as a
 * tool result) should read or repeat. The policy still decides everything —
 * see `recordIssueEvent` — and the handoff stored with a ticket keeps the full
 * history for the support team. */
export function issueView(issue:SupportIssue) {return {id:issue.id,topic:issue.topic,summary:issue.summary,state:issue.state,ticketId:issue.ticketId};}

/** The automatic handoff's body: what was asked, what TRAX checked, what is left.
 *
 * It is labelled as a generated summary because it is one — no support agent
 * wrote it, and the tenant has not typed a message into a ticket composer. */
export function troubleshootingSummary(issue:SupportIssue):string {
  const question=issue.excerpts.find(e=>e.role==='user')?.content??issue.summary;
  const lines=['TRAX troubleshooting summary — generated by TRAX for this issue. It is not a message written by a support agent, and the requester has not added their own message yet.','',`Question: ${question}`];
  if(issue.summary&&issue.summary!==question)lines.push(`Issue: ${issue.summary}`);
  lines.push('',issue.checks.length?'Checks TRAX completed:':'Checks TRAX completed: none were available for this issue.');
  for(const check of issue.checks.slice(-6))lines.push(`- ${check.tool} (${check.status}, ${check.observedAt}): ${check.findings.slice(0,3).join(' ')||'no findings recorded'}`);
  const attempted=[...new Set(issue.events.map(e=>e.reason))].filter(r=>r!=='verified_progress'&&r!=='user_resolved');
  if(attempted.length)lines.push('',`Why TRAX could not close it: ${attempted.map(r=>r.replace(/_/g,' ')).join(', ')}.`);
  if(issue.unknowns.length){lines.push('','Still unresolved:');for(const unknown of issue.unknowns.slice(0,6))lines.push(`- ${unknown}`);}
  if(issue.records.length)lines.push('',`Records referenced: ${issue.records.slice(0,6).map(r=>`${r.kind} ${r.id}`).join(', ')}.`);
