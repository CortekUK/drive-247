...
export const MODEL_TOOLS:ModelTool[]=[
  ...
  tool('get_integration_status','Whether each of this account’s integrations is connected, and what its stored state says: Stripe, Square, Twilio SMS and calling, Bonzah, INSHUR, Xero, Zoho, Tesla Fleet and custom domains. Returns the connection health, the mode, the account it is connected to, an error the provider recorded where one is stored, and when that state was recorded. This is STORED state read from the account’s own settings — no integration is contacted, and the portal does not contact them either. Use for "is Stripe connected", "which integrations are set up", "why is Xero not syncing".',{}),
