...
<head>
    <meta charset="utf-8">
    <title>Booking Cancelled - DRIVE 247</title>
</head>
                    ...
                    <tr>
                        <td style="padding: 30px;">
                            ...
                            <p style="margin: 0 0 20px; color: #444; line-height: 1.6; font-size: 16px;">
                                We're writing to confirm that your booking with DRIVE 247 has been cancelled.
                            </p>
                                ...
                                <tr>
                                    <td style="padding: 20px;">
                                        <table role="presentation" style="width: 100%; border-collapse: collapse;">
                                            ...
                                            ${data.vehicleReg ? `<tr><td style="padding: 8px 0; color: #666; font-size: 14px;">Registration:</td><td style="padding: 8px 0; color: #1a1a1a; font-weight: 600; font-size: 14px; text-align: right;">${data.vehicleReg}</td></tr>` : ''}
                                        </table>
                                    </td>
                                </tr>
                            </table>
                            <table role="presentation" style="width: 100%; border-collapse: collapse; background: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 0 8px 8px 0; margin-bottom: 25px;">
                                <tr>
                                    <td style="padding: 20px;">
                                        <h3 style="margin: 0 0 10px; color: #92400e; font-size: 14px;">Reason for Cancellation</h3>
                            ...
                            <table role="presentation" style="width: 100%; border-collapse: collapse; background: #ecfdf5; border-left: 4px solid #10b981; border-radius: 0 8px 8px 0; margin-bottom: 25px;">
                                <tr>
                                    <td style="padding: 20px;">
                                        <h3 style="margin: 0 0 10px; color: #047857; font-size: 16px;">Refund Information</h3>
                                        ${refundMessage}
                                    </td>
                                </tr>
                            </table>
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Sending booking cancelled notifications for:', data.bookingRef);
    ...
    if (data.tenantId) {
      try {
        ...
        const rendered = await renderEmail(supabase, data.tenantId, 'booking_cancelled', templateData);
