...
serve(async (req) => {
  ...
  try {
    ...
    // the SMS, and the {{vehicle_reg}} substitution into the tenant's own
    // lockbox_templates.
    ...
    // Fetch tenant branding if tenantId provided
    if (data.tenantId) {
...
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Arial, sans-serif; background-color: #f5f5f5;">
    ...
    // Resolve which channels actually fire.
    //
    // A WhatsApp leg used to sit alongside these two. It has been removed, and a
    // tenant whose only configured lockbox channel was that one — or whose SMS
    // leg cannot run because no phone number reached us — would otherwise be
    // left with nothing at all. Falling back to email is not cosmetic: this
    // message carries the code that opens the box holding the car keys, so
    // "send nothing" is never an acceptable outcome.
    const emailRequested = data.sendEmail !== false; // default true for backward compat
    const smsRequested = data.sendSms !== false;     // default true for backward compat
    const smsPossible = smsRequested && !!data.customerPhone;
    const shouldSendEmail = emailRequested || !smsPossible;
    const shouldSendSms = smsRequested;
    ...
    if (shouldSendEmail) {
      ...
      if (!emailRequested) {
        console.log('Email was not requested but is the only remaining channel — sent as fallback');
      }
    } else {
