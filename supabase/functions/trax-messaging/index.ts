...
import { handleMessaging, type MessagingDatabase, type MessagingStorage } from '../trax-support/support/messaging.ts';
...
Deno.serve(async(req:Request)=>{
  ...
  const response=await handleMessaging(req,{reads:createSupportReads(db as unknown as SupportDatabase),db:db as unknown as MessagingDatabase,enabled:(Deno.env.get('TRAX_SUPPORT_STORAGE')??'enabled')==='enabled',storage}); // Defaults on: the project is at its secret limit; a secret still overrides.
