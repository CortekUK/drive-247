...
async function verifySuperAdmin(supabase: any, userId: string): Promise<boolean> {
  ...
  // is_active too — a deactivated super admin must not start migrations.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Platform ToS on the migration path.
    //
    // The link is GENERATED by a super admin, but it is the OPERATOR who opens
    // it and enters their card — so this is an operator commitment, not an
    // admin one, and it was the last operator-self-serve payment surface with
    // no acceptance gate. There is no in-app checkbox to attach here (the
    // operator arrives on Stripe's page from a link, never through the portal's
    // PricingCard), so consent can only be collected by Stripe itself.
    //
    // tos_version rides the metadata so subscription-webhook can stamp the
    // acceptance; the webhook only writes when it sees an actual consent
    // result, so with the gate disabled this metadata is inert rather than
    // recording an acceptance nobody gave.
    sessionMetadata.tos_version = PLATFORM_TOS_VERSION;

    // Same env gate as create-subscription-checkout: Stripe 400s this unless a
    // Terms of Service URL is configured on the account, and that has to be set
    // on uk/test, uk/live, uae/test AND uae/live. Enabling it before all four
    // are configured would break the migration for every tenant mid-cutover.
    const stripeTosConsentEnabled =
