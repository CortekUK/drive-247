  ...
  try {
    ...
    if (signature && secretCandidates.length > 0) {
      ...
      if (!verified) {
        ...
        // Fail-open health record. Never awaited for correctness — see
        // _shared/webhook-health.ts: a logging miss must not become an outage.
        await recordWebhookDelivery(supabase, {
          platform: "stripe", mode: "test", outcome: "rejected_signature",
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (session.metadata?.type === "payment_plan") {
          ...
          console.log("[TEST MODE] payment plan checkout:", session.id, planResult.status);
          break;
        }
    ...
    await recordWebhookDelivery(supabase, {
      platform: "stripe", mode: "test", outcome: "handled",
    ...
    // `supabase` is block-scoped to the try above and is NOT visible here, so
    // this path builds its own client. Still fail-open: the helper swallows.
    try {
      ...
      await recordWebhookDelivery(healthClient, {
        platform: "stripe", mode: "test", outcome: "failed",
