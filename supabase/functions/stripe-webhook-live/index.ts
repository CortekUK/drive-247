...
// ---------------------------------------------------------------------------
// Security-deposit authorisations vs booking pre-auths
// ---------------------------------------------------------------------------
/**
...
// ---------------------------------------------------------------------------
// payments.preauth_expires_at reconciliation
// ---------------------------------------------------------------------------
      ...
      case "checkout.session.completed": {
        ...
        if (isPreAuth) {
          ...
          if (preauthPiId) {
            ...
            console.warn(
              "[LIVE MODE] Pre-auth session exposed no PaymentIntent id; preauth_expires_at left at its floor:",
              session.id
            );
          }
          ...
          await settleBookingPayment(
            { supabase, stripe, stripeOptions, platformAccount, logPrefix: "[LIVE MODE]" },
            {
              rentalId,
