// MIRROR of apps/portal/src/lib/agreements-v2/validation.ts, for the
// agreements-v2 edge function. Deno cannot resolve the portal's `@/` alias, so
// the file is copied rather than imported, with exactly ONE line changed: the
// `OPERATOR_SIGNATURE_ATTR` import from lib/agreements-v2/types is replaced by
// the same constant declared inline. Everything after this banner must stay
// byte-identical to the portal file otherwise;
// tests/integrations/agreements-v2/validation-mirror.test.ts fails the moment
// the two drift. Change the portal file first, then re-copy it here.
...
const OPERATOR_SIGNATURE_ATTR = 'data-operator-signature';
