-- Explicitly qualify the requested email inside the correlated anti-join.
-- Without qualification, PL/pgSQL/PostgreSQL can resolve `email` to the
-- existing table column, causing every new address to appear already present.
