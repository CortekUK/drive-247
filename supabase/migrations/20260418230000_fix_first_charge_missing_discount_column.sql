-- generate_first_charge_for_rental selected `discount_amount` from the
-- invoices table, but that column doesn't exist — the function errored
-- "column 'discount_amount' does not exist" the first time it ran the
-- invoice-found branch for a new tenant. The original migration that
-- introduced this function had the same latent bug, which we carried
-- through when porting the ON CONFLICT syntax. Drop the column from the
-- SELECT list since it wasn't used in the body anyway.
...
BEGIN
  ...
  SELECT rental_fee, tax_amount, service_fee, security_deposit, insurance_premium, delivery_fee, extras_total
  INTO v_invoice
  FROM invoices
  WHERE rental_id = v_rental.id
