-- Fix delete_rental_cascade to handle FK dependencies between related tables
-- The issue: ledger_entries.payment_id references payments.id, so we must delete
-- ledger_entries BEFORE payments, not in arbitrary order
