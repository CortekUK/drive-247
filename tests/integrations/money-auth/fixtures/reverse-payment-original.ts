...
//
// The request handler of supabase/functions/reverse-payment/index.ts exactly
// as it stood at commit 68e90fbd — before it verified its caller, before it
// refused a refunded payment, and before the handler moved into core.ts.
//
// tests/integrations/money-auth/reverse-payment-contract.test.ts drives this
// copy and the real core.ts with the same requests and the same recording fake
// client, and requires identical responses and identical writes for every
// AUTHORISED caller — so the new guard is provably invisible to the operators
// it lets through.
//
// Everything between the marker lines is verbatim (lines 5–9 and 18–280 of the
// file at 68e90fbd: `corsHeaders`, and the body of the `serve(async (req) => { … })`
// callback). Only the wrapper is added: the callback becomes a function, and
// `Deno.env` and `createClient` are handed in instead of being globals/imports.
import { hasProcessorHandle } from "../../../../supabase/functions/_shared/payments/predicates.ts";

export function makeOriginalReversePayment(
...
// ---- verbatim from reverse-payment/index.ts @ 68e90fbd (lines 5–9) ----
const corsHeaders = {
...
// ---- verbatim from reverse-payment/index.ts @ 68e90fbd (lines 18–280) ----
