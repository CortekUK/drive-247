...
// RUNG 3 — MOVING MONEY.
//
// D247_LIVE_TESTS unlocks reads. D247_LIVE_ALLOW_WRITES unlocks writes to OUR
// database — an auth.users row, a tenant row. Junk, but junk we own, on a
// project we chose, deletable with one SQL statement.
//
// A Stripe refund is neither of those. It is a write to a THIRD PARTY with no
// DELETE: the moment `refunds.create` returns, money has left an operator's
// Stripe balance and is on its way to a cardholder's bank, and no flag,
// migration or rollback in this repository can call it back. A refund test that
// runs against a real charge is not a test of a refund. It IS a refund.
//
// So the Stripe money cases need a third, separate yes — and, unlike the signup
// writes, they must also state which Stripe world they are in, because the
// Supabase guards above say nothing about that. A non-production Supabase
// project can perfectly well hold a tenant configured with LIVE Stripe keys;
// `stripe_mode` is a per-tenant column, not a property of the database.
//
//   D247_LIVE_ALLOW_MONEY_MOVEMENT=1   the third yes
//   D247_LIVE_STRIPE_MODE=test         an explicit statement that the tenant
//                                      owning the fixtures is in Stripe TEST
//                                      mode
//
// Absent means refused, both of them. Fail closed (V2_PLAN.md §2): "nobody
// said" must never be read as "it is fine". And like D247_LIVE_PROJECT_REF, the
// mode declaration can only ever NARROW — `liveStatus()` has already run and
// already thrown by the time it is read, so nothing here can excuse a
 ...
 *
 * `it.skipIf(...)` is evaluated at COLLECTION time, before any test body runs,
 * so it needs a plain boolean that cannot throw. The real checks live in
 * `liveMoneyGate()`, which runs inside the test where a throw is a red test
 * rather than an unhandled collection error that takes the whole file out.
 */
export function liveMoneyMovementRequested(): boolean {
  return env("D247_LIVE_ALLOW_MONEY_MOVEMENT") === "1";
}

function moneyRefusal(lines: string[]): ProductionTargetError {
  return new ProductionTargetError(
    [
      "",
      "  ############################################################",
      "  #  REFUSING TO RUN A TEST THAT MOVES REAL MONEY            #",
      "  ############################################################",
      "",
      ...lines.map((l) => `  ${l}`),
      "",
      "  A Stripe refund cannot be undone by this repository. Configure the run",
      "  completely, or leave the money cases skipped — which is the default and",
      "  is the correct state for CI.",
      "",
    ].join("\n"),
  );
}
...
/**
 * Resolve permission to run a case that can move money at Stripe.
 *
 * Deliberately shaped like `liveStatus()`: "you did not ask for this" is a
 * SKIP, and everything after that point is a hard throw. Half-configuring a run
 * that can move money is worse than a red build, so it is made into one.
 */
export function liveMoneyGate(): MoneyGate {
  if (!liveMoneyMovementRequested()) {
    return {
      allowed: false,
      reason:
        "Money-movement cases are off (D247_LIVE_ALLOW_MONEY_MOVEMENT is not 1). " +
        "The contract half of this case still ran — see tests/integrations/stripe/README.md.",
    };
  }

  // Rung 1, inherited rather than re-implemented. This is the call that carries
  // the production refusal, and it throws; putting it first means a money run
  // aimed at hviqoaokxvlancmftwuo dies here with that banner and never reaches
  // the Stripe-specific checks below.
  ...
  if (!status.enabled) {
    throw moneyRefusal([
      ...
      `  (${status.reason})`,
      "",
      "You have said money may move and then not said where. That combination is",
      "never a default — set D247_LIVE_TESTS=1 and an explicit target, or unset",
      "D247_LIVE_ALLOW_MONEY_MOVEMENT.",
    ]);
  }

  // Rung 2. Moving money always writes: the refund row, the payment status, the
  // ledger entry. A run that claims it may move money but may not write is
  // incoherent, and the incoherent half is the permissive one.
  if (!liveWritesAllowed()) {
    throw moneyRefusal([
      "D247_LIVE_ALLOW_MONEY_MOVEMENT=1 but D247_LIVE_ALLOW_WRITES is not 1.",
      "",
      "Every refund writes — payments.refund_amount, payments.status and a",
      "ledger_entries row all change. Money movement is a strict superset of",
      "writing, so it cannot be granted while writing is withheld.",
    ]);
  }

  const declaredMode = env("D247_LIVE_STRIPE_MODE");
  if (declaredMode !== "test") {
    throw moneyRefusal([
      declaredMode
        ? `D247_LIVE_STRIPE_MODE is "${declaredMode}". Only "test" is accepted.`
        : "D247_LIVE_STRIPE_MODE is not set.",
      "",
      "The Supabase guards above prove which DATABASE this run points at. They",
      "prove nothing about Stripe: `tenants.stripe_mode` is a per-tenant column,",
      "so a non-production project can hold a tenant wired to live Stripe keys —",
      "and process-refund reads that column, not this variable, when it decides",
      "which Stripe account to refund from.",
      "",
      "So this is a declaration, not a detection, and it is the one thing a human",
      "has to assert by hand. Unset is refused rather than assumed: the failure",
      "direction of a wrong guess here is somebody's real money.",
    ]);
  }
